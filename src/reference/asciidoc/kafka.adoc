...
NOTE: The admin does not alter existing topics; it will log (INFO) if the number of partitions don't match.
...
IMPORTANT: The `Jackson` `ObjectMapper` (even if provided) will be enhanced to support deserializing `org.springframewor.utils.MimeType` objects, often used in the `spring-messaging` `ContentType` header.
