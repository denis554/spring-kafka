...
The `brokerProperties` and `brokerPropertiesLocation` attribute of `@EmbeddedKafka` supports standard Spring property placeholder resolution from the `Environment`.
The `Environment` contains system properties, the system environment, _and_ properties from the classpath resource `/test.properties`, and all of these property sources are consulted when resolving property placeholders.
For example:
[source, java]
----
...
----
In this example, the property placeholders `${kafka.broker.logs-dir}` and `${kafka.broker.port}` are resolved from the Spring `Environment`.
Additional broker properties are loaded from the `broker.properties` classpath resource specified by the `brokerPropertiesLocation`.
Property placeholders are resolved for the `brokerPropertiesLocation` URL _and_ for any property placeholders found in the resource.
